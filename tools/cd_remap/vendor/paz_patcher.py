...
def _write_manifest(backup: Path, game_dir: Path) -> None:
    """Record which game directory this backup belongs to. Skips if exists."""
    manifest = backup / "manifest.json"
    if manifest.exists():
        return
    import json as _json
    manifest.write_text(_json.dumps({"game_dir": str(game_dir.resolve())}))


def _check_manifest(backup: Path, game_dir: Path) -> None:
    """Raise ValueError if backup was created for a different game directory."""
    import json as _json
    manifest = backup / "manifest.json"
    if not manifest.exists():
        return  # legacy backup — allow
    stored = Path(_json.loads(manifest.read_text())["game_dir"]).resolve()
    if stored != game_dir.resolve():
        raise ValueError(
            f"Backup was created for {stored}, "
            f"but restore requested for {game_dir.resolve()}. "
            f"Delete the backup manually if you want to proceed."
        )


def _restore_backup_files(backup: Path, game_dir: Path, paz_indices: set[int]) -> None:
    """Restore PAZ/PAMT/PAPGT from backup after a failed write."""
    rels = [f"{PAZ_FOLDER}/0.pamt", "meta/0.papgt"]
    rels += [f"{PAZ_FOLDER}/{i}.paz" for i in paz_indices]
    for rel in rels:
        src = backup / rel
...
def apply_paz_patch(patches: list[tuple[str, bytes]], game_dir: Path, *, progress_cb=None) -> dict:
    ...
    _write_manifest(backup, game_dir)
    ...
    try:
        ...
        _restore_backup_files(backup, game_dir, paz_indices)
        raise
...
def remove_paz_patch(game_dir: Path, *, progress_cb=None) -> dict:
    ...
    try:
        _check_manifest(backup, game_dir)
